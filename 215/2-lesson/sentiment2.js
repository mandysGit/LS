...
function sentiment(text) {
  let posWords = text.match(positiveRegex).map(word => word.toLowerCase());
  let negWords = text.match(negativeRegex).map(word => word.toLowerCase());
...
function getCountAndWords(textArr, regex) {
  let count = 0;
  let result = [];
